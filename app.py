...
with tab1:
    ...
    with st.form("add_room"):
        ...
        ach = st.selectbox("Air Changes per Hour", [0.3, 0.5, 0.8, 1.0, 1.5], index=1)
        occupants = st.number_input("Occupants", value=2)
    ...
    if st.session_state.rooms:
        ...
        with col_b:
            if st.button("Share Results"):
                share_text = f"VetCool Multi-Room Calculation\nTotal: {total_btu} BTU/hr\nRooms: {len(st.session_state.rooms)}\nLink: https://your-app-link.streamlit.app"
                st.code(share_text)
                st.success("Copied to clipboard!")

# PDF & Download (for total)
if st.session_state.rooms and st.button("Download Full Multi-Room PDF"):
    # Create PDF with all rooms
    pdf = FPDF()
    pdf.add_page()
    pdf.set_font("Arial", "B", 16)
    pdf.cell(0, 10, "VetCool Multi-Room HVAC Report", ln=True, align="C")
    # ... (add more details if needed)
    filename = "vetcool_multiroom_report.pdf"
    pdf.output(filename)
...
with tab2:
    ...
    st.write("Add each room one by one. The app calculates grand total automatically.")
...
def calculate_heating_load(data):
    # ... same as before
    pass  # Replace with your real function if needed
