...
# --- ROUTE B: AUTHENTICATED CORE APP ---
    ...
    with tab1:
        ...
        with col2:
            ...
            with c_sub4:
                init_ach = float(active_override["ach"] if active_override else 0.5)
                ach = st.number_input(ctx["tightness"], value=init_ach, step=0.1, help=ctx["tightness_help"])
                
                init_shgc = float(active_override["shgc"] if active_override else 0.40)
                shgc = st.number_input(ctx["shgc_lbl"], value=init_shgc, step=0.05, help=ctx["shgc_help"])

        # Package data dictionary for calculation pipelines
        data_payload = {
            "location": location, "mode": mode, "t_indoor": t_indoor, "t_outdoor": t_outdoor,
            "moisture_grains": moisture_grains, "cltd_wall": cltd_wall, "cltd_roof": cltd_roof,
            "area_walls": area_walls, "u_walls": u_walls, "area_windows": area_windows, "u_windows": u_windows,
            "area_roof": area_roof, "u_roof": u_roof, "volume": volume, "ach": ach,
            "occupants": occupants, "shgc": shgc, "safety_factor": safety_factor
        }
        ...
        if st.button(ctx["btn_calc"], type="primary"):
            if mode == "Heating Load":
                result = calculate_heating_load(data_payload)
                result['tons'] = 0.0
            else:
                result = calculate_cooling_load(data_payload)
            
            # Save metrics safely to the linked cloud ledger
            save_calculation(proj_name, data_payload, result, lang, current_user["id"])
            st.session_state["last_computed_results"] = result
            st.session_state["last_computed_payload"] = data_payload
            st.success("Load profile computed and synchronized to secure storage engine!")

        # Display Metrics Output UI Layout Layer if calculation exists
        if "last_computed_results" in st.session_state:
            res = st.session_state["last_computed_results"]
            pay = st.session_state["last_computed_payload"]
            
            st.markdown("---")
            m_col1, m_col2, m_col3 = st.columns(3)
            with m_col1:
                lbl = ctx["heat_capacity"] if pay["mode"] == "Heating Load" else ctx["cool_capacity"]
                val = f"{res['total_btu_hr']:,} BTU/hr"
                st.markdown(f'<div class="metric-card"><h5>{lbl}</h5><h3>{val}</h3></div>', unsafe_allow_html=True)
            with m_col2:
                st.markdown(f'<div class="metric-card"><h5>{ctx["circ_target"]}</h5><h3>{res["cfm"]:,} CFM</h3></div>', unsafe_allow_html=True)
            with m_col3:
                duct_size = get_duct_recommendation(res['cfm'], lang)
                st.markdown(f'<div class="metric-card"><h5>{ctx["suggested_duct"]}</h5><h3>{duct_size}</h3></div>', unsafe_allow_html=True)
            
            st.markdown("<br>", unsafe_allow_html=True)
            
            # THE AUTOMATED MAIL ROUTER ACTION
            if st.button(ctx["btn_pdf"]):
                with st.spinner("Compiling high-speed PDF vectors & routing to SMTP infrastructure..."):
                    try:
                        # 1. Compile report document directly to local worker file path
                        report_file = generate_pdf_report(pay, res, pay["mode"], lang, ctx, proj_name)
                        
                        # 2. Extract logged-in session user's email routing profile target
                        target_recipient = current_user["email"]
                        
                        # 3. Fire SMTP packet out via environmental transport layers
                        mail_success, mail_log = send_pdf_email(target_recipient, report_file, proj_name)
                        
                        if mail_success:
                            st.success(f"🚀 Proposal dispatched successfully to **{target_recipient}**!")
                            if os.path.exists(report_file):
                                os.remove(report_file)
                        else:
                            st.error(f"Mail Transmission Interrupted: {mail_log}")
                            
                    except Exception as pdf_err:
                        st.error(f"{ctx['pdf_fault']}: {str(pdf_err)}")

    with tab2:
        st.subheader(ctx["method_title"])
        st.write(ctx["method_body"])
